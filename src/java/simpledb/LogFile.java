...
public class LogFile {
    ...
    private void rollback(long tid) throws IOException {
        if (!tidToFirstLogRecord.containsKey(tid)) throw new NoSuchElementException("This tid is not in the log.");
        // find first txn
        ...
        while (true) {
            try {
                // go over from the first record entry, go through each record
                int type = raf.readInt();
                long record_tid = raf.readLong();
                switch (type) {
                    case UPDATE_RECORD:
                        Page before = readPageData(raf);
                        Page after = readPageData(raf);
                        // add to the page list of the current trans id
                        if (record_tid == tid) {
                            pageList.add(before);
                        }
                        break;
    ...
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                Long tid, recordOffset;
                List<Long> completedTransactions = new ArrayList<>();

                // search from bottom up to the checkpoint.
                // make sure committed changes are unchanged.
                // uncommitted changes are aborted.
                while (cpLoc != offset) {
                    raf.seek(offset - LONG_SIZE);
                    offset = raf.readLong();
                    if (offset == -1) break; // invalid offset for this log record
                    raf.seek(offset);
                    int type = raf.readInt();
                    switch (type) {
                        case BEGIN_RECORD:
                        ...
                        case CHECKPOINT_RECORD:
                            raf.readLong();
                            for (int numXactions = raf.readInt(); numXactions > 0; --numXactions) {
                                tid = raf.readLong();
                                recordOffset = raf.readLong();
                                if (!completedTransactions.contains(tid))
                                    tidToFirstLogRecord.put(tid, recordOffset);
                ...
                for (Object incompleteTid: tidToFirstLogRecord.keySet().toArray()) {
                    logAbort(new TransactionId((Long) incompleteTid));
                }
            }
        }
    }
