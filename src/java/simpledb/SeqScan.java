...
public class SeqScan implements OpIterator {
    ...
    public void open() throws DbException, TransactionAbortedException {
        // some code goes here
        this.dbFileIterator = Database.getCatalog().getDatabaseFile(this.tableid).iterator(tid);
    ...
    public TupleDesc getTupleDesc() {
        ...
        String[] fieldAr = new String[tupleDesc.itemAr.size()];
        Type[] types = new Type[tupleDesc.itemAr.size()];
        for (int i = 0; i < tupleDesc.itemAr.size(); i++) {
            String fieldName = tupleDesc.getFieldName(i);
            if (fieldName == null) {
                fieldName = "null";
            }
            fieldAr[i] = this.getAlias() + "." + fieldName;
